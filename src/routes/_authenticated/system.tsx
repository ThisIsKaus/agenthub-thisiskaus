  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "System — AgentHub Remote" },
      {
        ...
        content:
          "How AgentHub is put together: the two planes, the three routing lanes, the posture rules and the open risk register.",
      },
      ...
      <header className="pb-8">
        ...
        <p className="mb-7 max-w-[62ch] text-[15px] leading-relaxed text-muted-foreground">
          ...
          <strong className="font-medium text-paper">
            Production lives in the cloud; the machine pushes, it never serves.
          </strong>{" "}
          This workspace is the only interaction and configuration surface — nothing here is a rendered file or a
          hand-written claim.
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          <StatePill
            ...
            label="Router"
            value={`${services.aliases ?? 0} aliases`}
          />
          <StatePill
            tone={(health.failed ?? 0) > 0 ? "risk" : (health.warnings ?? 0) > 0 ? "watch" : "ok"}
            label="Self-test"
            value={`${health.passed ?? 0}/${health.warnings ?? 0}/${health.failed ?? 0}`}
          />
          <StatePill
            tone={(factory.wip ?? 0) >= (factory.limit ?? 2) ? "watch" : "ok"}
            label="Factory"
            value={`${factory.wip ?? 0}/${factory.limit ?? 2} wip`}
          />
        </div>
      </header>
          ...
          >
            <defs>
              <marker id="sys-ar" markerWidth="9" markerHeight="9" refX="8" refY="4.5" orient="auto">
                <path d="M0,0 L9,4.5 L0,9 Z" fill="#5E5E66" />
              </marker>
            ...
            <text x="95" y="203" textAnchor="middle" fill="#8E8E96" fontFamily="Geist Mono,monospace" fontSize="11">mail · calendar · files</text>
            ...
            <line x1="172" y1="190" x2="211" y2="190" stroke="#5E5E66" strokeWidth="1.5" markerEnd="url(#sys-ar)" />
            ...
            <text x="752" y="104" fill="#5E5E66" fontFamily="Geist Mono,monospace" fontSize="10">unlimited</text>
            ...
            <text x="752" y="206" fill="#5E5E66" fontFamily="Geist Mono,monospace" fontSize="10">marginal</text>
            ...
            <text x="752" y="308" fill="#5E5E66" fontFamily="Geist Mono,monospace" fontSize="10">logged per request</text>
            <path d="M836 88 L856 88 L856 292 L836 292" fill="none" stroke="#26262A" />
            <path d="M836 190 L856 190" stroke="#26262A" />
            <line x1="856" y1="190" x2="906" y2="190" stroke="#5E5E66" strokeWidth="1.5" markerEnd="url(#sys-ar)" />
        ...
        <div className="border border-rule bg-panel">
          {PHASES.map(([id, title, state, ev]) => (
            <details key={id} className="border-b border-rule last:border-b-0">
              ...
              <p className="px-4 pb-4 pl-[64px] text-[13px] leading-relaxed text-muted-foreground">{ev}</p>
            </details>
          ))}
        </div>
      </Section>
          ...
          {RISKS.map((r) => (
            <div key={r.title} className="grid grid-cols-[3px_1fr] gap-4 border-b border-rule py-4 last:border-b-0">
              ...
              <div>
                ...
                <p className="mt-1 max-w-[76ch] text-sm leading-relaxed text-muted-foreground">{r.detail}</p>
